...
let intro:String = "Hello, everyone! Serendipitous day we're having!"
 
var other = intro.components(separatedBy: " ")
var newword = ""
var wordRet = ""
var idx = 0
var columnWidth = 15
...
for i in 0 ..< other.count{
        if idx == (other.count-1){
            wordRet += other[idx]+"\n"
            break
        }
        if other[idx].count >= columnWidth{
            wordRet += other[idx] + "\n"
        }else{
            newword = other[idx] + other[idx+1]
            if newword.count >= columnWidth {
                wordRet += other[idx] + "\n"
            }else{
                wordRet += newword + "\n"
                idx += 1
